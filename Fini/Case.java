...
public class Case implements Comparable {
    ...
    static final ArrayList<Piece> pieces = new ArrayList<>(Arrays.asList(
            Piece.Moteur,Piece.Energie,Piece.Gouvernail,Piece.Helice,Piece.Ind,Piece.Ind,Piece.Ind,Piece.Ind
    ));

    public Case(int x, int y, int niveau,Zone type) {

        switch (type){
            case Crash,Piste,Tunnels,Oasis,Normal,FakeOasis -> {
                this.x = x;
                this.y = y;
                this.niveau = niveau;
            }case Oeil ->{
